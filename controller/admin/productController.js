...
const addProduct = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Product,[ 'handle' ],dataToCreate,'INSERT');
...
const bulkInsertProduct = async (req, res)=>{
  try {
    ...
    if (dataToCreate !== undefined && dataToCreate.length){
      let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Product,[ 'handle' ],dataToCreate,'BULK_INSERT');
      if (checkUniqueFields.isDuplicate){
        return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
      }
...
const updateProduct = async (req, res) => {
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Product,[ 'handle' ],dataToUpdate,'UPDATE', query);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
...
const bulkUpdateProduct = async (req, res)=>{
  try {
    ...
    let checkUniqueFields = await utils.checkUniqueFieldsInDatabase(Product,[ 'handle' ],dataToUpdate,'BULK_UPDATE', filter);
    if (checkUniqueFields.isDuplicate){
      return res.validationError({ message : `${checkUniqueFields.value} already exists.Only unique ${checkUniqueFields.field} are allowed.` });
    }
