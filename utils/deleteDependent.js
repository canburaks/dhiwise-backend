...
let Variant = require('../model/variant');
...
const deleteVariant = async (filter) =>{
  try {
    let response  = await dbService.destroy(Variant,filter);
    return response;
  } catch (error){
...
const deleteArtist = async (filter) =>{
  try {
    let response  = await dbService.destroy(Artist,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteTag = async (filter) =>{
  try {
    let response  = await dbService.destroy(Tag,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteCollection = async (filter) =>{
  try {
    let response  = await dbService.destroy(Collection,filter);
    return response;
  } catch (error){
    throw new Error(error.message);
  }
...
const deleteProduct = async (filter) =>{
  try {
    ...
    if (product && product.length){
      ...
      const variantFilter = { $or: [{ product : { $in : product } }] };
      const variantCnt = await dbService.destroy(Variant,variantFilter);

      let deleted  = await dbService.destroy(Product,filter);
      let response = { variant :variantCnt.length, };
      return response; 
    } else {
      return {  product : 0 };
    }
...
const deleteUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const artistFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const artistCnt = await dbService.destroy(Artist,artistFilter);
      ...
      const collectionCnt = await dbService.destroy(Collection,collectionFilter);
      ...
      let response = {
        artist :artistCnt.length,
        tag :tagCnt.length,
        collection :collectionCnt.length,
...
const countVariant = async (filter) =>{
  try {
    const variantCnt =  await dbService.count(Variant,filter);
    return { variant : variantCnt };
  } catch (error){
    throw new Error(error.message);
  }
};

const countArtist = async (filter) =>{
  try {
    const artistCnt =  await dbService.count(Artist,filter);
    return { artist : artistCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countTag = async (filter) =>{
  try {
    const tagCnt =  await dbService.count(Tag,filter);
    return { tag : tagCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const countCollection = async (filter) =>{
  try {
    const collectionCnt =  await dbService.count(Collection,filter);
    return { collection : collectionCnt };
  } catch (error){
    throw new Error(error.message);
  }
};
...
const countProduct = async (filter) =>{
  try {
    ...
    if (product && product.length){
      ...
      const variantFilter = { $or: [{ product : { $in : product } }] };
      const variantCnt =  await dbService.count(Variant,variantFilter);

      let response = { variant : variantCnt, };
      return response; 
    } else {
      return {  product : 0 };
    }
...
const countUser = async (filter) =>{
  try {
    ...
    if (user && user.length){
      ...
      const artistFilter = { $or: [{ addedBy : { $in : user } },{ updatedBy : { $in : user } }] };
      const artistCnt =  await dbService.count(Artist,artistFilter);
...
const softDeleteVariant = async (filter,updateBody) =>{  
  try {
    const variantCnt =  await dbService.update(Variant,filter);
    return { variant : variantCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteArtist = async (filter,updateBody) =>{  
  try {
    const artistCnt =  await dbService.update(Artist,filter);
    return { artist : artistCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteTag = async (filter,updateBody) =>{  
  try {
    const tagCnt =  await dbService.update(Tag,filter);
    return { tag : tagCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteCollection = async (filter,updateBody) =>{  
  try {
    const collectionCnt =  await dbService.update(Collection,filter);
    return { collection : collectionCnt };
  } catch (error){
    throw new Error(error.message);
  }
...
const softDeleteProduct = async (filter,updateBody) =>{  
  try {
    ...
    if (product.length){
      ...
      const variantFilter = { '$or': [{ product : { '$in' : product } }] };
      const variantCnt = await dbService.update(Variant,variantFilter,updateBody);
...
const softDeleteUser = async (filter,updateBody) =>{  
  try {
    ...
    if (user.length){
      ...
      const artistFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const artistCnt = await dbService.update(Artist,artistFilter,updateBody);

      const tagFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const tagCnt = await dbService.update(Tag,tagFilter,updateBody);

      const collectionFilter = { '$or': [{ addedBy : { '$in' : user } },{ updatedBy : { '$in' : user } }] };
      const collectionCnt = await dbService.update(Collection,collectionFilter,updateBody);
